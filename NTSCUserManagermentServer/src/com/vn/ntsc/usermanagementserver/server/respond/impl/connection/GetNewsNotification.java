 ...
 * @author Admin
 */
public class GetNewsNotification implements IApiAdapter {

    @Override
    public Respond execute(JSONObject obj, Date time) {
        ListEntityRespond respond = new ListEntityRespond();
        List<IEntity> result;
        Util.addDebugLog("Test for GetNewsNotification request"+obj);
        try {
...
//            Collection<String> deactiveList = BlackListManager.toList();
            ...
            result = NotificationDAO.getNotificationListByType(userId, date.getTime(), take, blockList, Constant.NOTIFICATION_TYPE_VALUE.AUTO_NEWS);
            UserDAO.updateNewsNotificationReadTime(userId,time.getTime());// edit LongLT 25Aug2016
