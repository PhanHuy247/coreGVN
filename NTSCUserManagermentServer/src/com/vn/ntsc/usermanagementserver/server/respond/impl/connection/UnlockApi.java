 ...
 * @author RuAc0n
 */
public class UnlockApi implements IApiAdapter {
...
//            int point = userDAO.getPoint(userId);
            ...
            int price = ConnectionPrice.getBadConnectionPrice(String.valueOf(ActionType.unlock_backstage), userId).senderPrice;
            if(price == 0){
            ...
            if (!UnlockDAO.isBackStageUnlock(userId, friendId)) {
                if (UserDAO.checkUser(friendId)) {
                    if (!BlockUserManager.isBlock(userId, friendId)) {
                        ...
                        //int number = 3;
                        if (number > 0) {
                            ActionResult actionResult = ActionManager.doExchangeOldVersion(userId, ActionType.unlock_backstage, friendId, ActionType.unlock_backstage_bonus, time, ip);
