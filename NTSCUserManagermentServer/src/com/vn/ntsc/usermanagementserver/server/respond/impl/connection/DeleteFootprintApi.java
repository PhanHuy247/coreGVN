 ...
 */
public class DeleteFootprintApi implements IApiAdapter{

    @Override
    public Respond execute(JSONObject obj, Date time) {
        Util.addDebugLog("Test obj from DeleteFootprintApi "+obj.toJSONString());
        EntityRespond respond = new EntityRespond();
        try {
            String notiId = (String) obj.get("footprint_id");
            if(notiId == null){
                respond = new EntityRespond(ErrorCode.NO_CONTENT);
            }
            else{
                MyFootPrintDAO.removeFootprint(new ObjectId(notiId));
