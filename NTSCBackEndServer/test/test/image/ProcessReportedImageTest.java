...
import data.provider.ProcessReportImageDataPro;
 ...
 * @author duyetpt
 */
public class ProcessReportedImageTest {

    Respond respond;
    Image image;
    String buzzId;

    public ProcessReportedImageTest() {
    }
...
//        Actor.removeUser();
        System.out.println("test imageId ;" + image.imageId);
    ...
    public void initData(int type, int imageFlag, int reportFlag,
        ...
        All.api = APIManager.getApi(API.PROCESS_REPORTED_IMAGE);
        All.put(ParamKey.IMAGE_ID, image.imageId);
        All.put(ParamKey.TYPE, (long) type);
    }

    @Test(dataProviderClass = ProcessReportImageDataPro.class, dataProvider = ProcessReportImageDataPro.DATA_NAME)
    ...
    public void doTest(int type, int prePbImage, int preBuzzNum, int preBackStage) throws EazyException {
        ...
        if (image.imageType == Constant.IMAGE_TYPE_VALUE.IMAGE_BACKSTAGE && type == 1 && checkReportFlag(image) == false
                && image.appFlag == Constant.REVIEW_STATUS_FLAG.APPROVED) {
            Image ima = ImageDAO.getImageInfor(image.imageId);
            assertEquals("ok", (Integer) flag, ima.reportFlag);
        ...
        if (image.imageType == Constant.IMAGE_TYPE_VALUE.IMAGE_PUBLIC && checkReportFlag(image) == false
                && image.appFlag == Constant.REVIEW_STATUS_FLAG.APPROVED) {
            Image ima = ImageDAO.getImageInfor(image.imageId);
            assertEquals("ok", (Integer) flag, ima.reportFlag);
            ...
            int UserBuzzAppFlag = type == 1 ? 1 : -1;
