...
import com.vn.ntsc.backend.dao.log.LogOnlineAlertDAO;
 ...
 * @author RuAc0n
 */
public class SearchLogOnlineAlertApi implements IApiAdapter {
    ...
    public Respond execute(JSONObject obj) {
        ...
        try {
            ...
            Long alertType = Util.getLongParam(obj, ParamKey.ALERT_TYPE);
            Long frequency = Util.getLongParam(obj, ParamKey.ALERT_FREQUENCY);
            ...
            SizedListData data = LogOnlineAlertDAO.listLog(listRequestId, listPartnerId, fromTime, toTime, alertType, frequency,
                    sort, order, skip, take, isCsv);
            if (isCsv) {
                String api = Util.getStringParam(obj, ParamKey.API_NAME);
                return LogProcess.createCSV(api, csv, data.ll, null, new LogOnlineAlert());
            }
