 ...
 * @author RuAc0n
 */
public class SetConnectionPointSettingApi implements IApiAdapter {
...
//            CommunicationSettingData data = validate(obj);
            ...
            ConnectionPointSettingDAO.update(obj);
            ...
            for(int i = 0; i < DAOKeys.pairs_list.size(); i++){
                ...
                if (value == null) {
                    ...
                    if(!key.equals(DAOKeys.female_female)){
                     JSONObject caller = (JSONObject) value.get(SettingdbKey.CONNECTION_POINT_SETTING.SENDER);
                     Long callerPoint = (Long) caller.get(DAOKeys.value);
                     if(!isValid(callerPoint, min, max))
                        return new CommunicationSettingData(calculate(i, 4, 3, plusCode), type);
                     JSONObject potentialCaller = (JSONObject) value.get(SettingdbKey.CONNECTION_POINT_SETTING.POTENTIAL_CUSTOMER_SENDER);
                     ...
                     JSONObject receiver = (JSONObject) value.get(SettingdbKey.CONNECTION_POINT_SETTING.RECEIVER);
                     ...
                     JSONObject potentialReceiver = (JSONObject) value.get(SettingdbKey.CONNECTION_POINT_SETTING.POTENTIAL_CUSTOMER_RECEIVER);
