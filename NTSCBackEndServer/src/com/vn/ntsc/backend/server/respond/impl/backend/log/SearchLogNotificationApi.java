...
import com.vn.ntsc.backend.dao.log.LogNotificationDAO;
 ...
 * @author RuAc0n
 */
public class SearchLogNotificationApi implements IApiAdapter {
...
//            toTime = getToTime(toTime);
            ...
            SizedListData data = LogNotificationDAO.listLog(listId, fromTime, toTime, type, sort, order, skip, take, isCsv);
            if (isCsv) {
                LogNotification.setPurpose(data.ll, null);
                String api = Util.getStringParam(obj, ParamKey.API_NAME);
                return LogProcess.createCSV(api, csv, data.ll, new LogNotification(), null);
            }
