 ...
 * @author RuAc0n
 */
public class SearchLogPointApi implements IApiAdapter {
...
//            toTime = getToTime(toTime);
            ...
            Long saleType = Util.getLongParam(obj, "sale_type");
            Long freePointId = Util.getLongParam(obj, "free_point_id");

            Long fromPoint = Util.getLongParam(obj, "from_point");
            Long toPoint = Util.getLongParam(obj, "to_point");
            ...
            SizedListData data = LogPointDAO.listLog(listId, fromTime, toTime, type, fromPoint, toPoint,
                    saleType, freePointId, sort, order, skip, take, isCsv);
            if (isCsv) {
                LogPoint.setPurpose(data.ll, null);
                String api = Util.getStringParam(obj, ParamKey.API_NAME);
                return LogProcess.createCSV(api, csv, data.ll, new LogPoint(), null);
            }
