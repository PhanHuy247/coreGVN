...
public class EditEmojiApi implements IApiAdapter{
    ...
    public Respond execute(JSONObject obj) {
        ...
        try{
            ...
            Set<String> setCode = EmojiDAO.getSetCode(id, catId, true);
            ...
            Set<String> listCode = EmojiDAO.getSetCode(id, catId, false);
            for (int i = 0; i < query.size(); i++) {
                String item = query.get(i).toString();
            ...
            Boolean isNameDuplicate = EmojiDAO.isEmojiNameExist(catId, name, id);
            
            if(inCatDuplicate || isNameDuplicate){
                respond = new EntityRespond(ErrorCode.EXISTS_DATA);
            }else{
                EmojiDAO.updateEmoji(id, fileId, catId, code, name, fileType, duplicate);
