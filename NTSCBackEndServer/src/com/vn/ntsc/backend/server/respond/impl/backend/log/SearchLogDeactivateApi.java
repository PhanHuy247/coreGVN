...
import com.vn.ntsc.backend.dao.log.LogDeactivateDAO;
 ...
 * @author RuAc0n
 */
public class SearchLogDeactivateApi implements IApiAdapter {
...
//            toTime = getToTime(toTime);
            ...
            SizedListData data = LogDeactivateDAO.listLog(listId, fromTime, toTime, sort, order, skip, take, isCsv);
            if (isCsv) {
                String api = Util.getStringParam(obj, ParamKey.API_NAME);
                return LogProcess.createCSV(api, csv, data.ll, new LogDeactivate(), null);
            }
