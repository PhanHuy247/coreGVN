 ...
 * @author RuAc0n
 */
public class ListReviewingBuzzApi implements IApiAdapter {
    ...
    public Respond execute(JSONObject obj) {
        ...
        try {
            ...
            SizedListData data = ReviewingBuzzDAO.getReviewingBuzz(userId, buzzId, sort, order, skip.intValue(), take.intValue());
            List<IEntity> ll = data.ll;
            List<String> list = new ArrayList<>();
            for (IEntity l : ll) {
                list.add(((ReviewingBuzz) l).userId);
            }
            Map<String, String> mapName = UserDAO.getUserName(list);
            for (IEntity l : ll) {
                String userName = mapName.get(((ReviewingBuzz) l).userId);
                ((ReviewingBuzz) l).userName = userName;
            }
