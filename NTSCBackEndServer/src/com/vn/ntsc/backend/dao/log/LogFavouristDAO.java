...
import com.vn.ntsc.backend.entity.impl.log.LogFavourist;
...
public class LogFavouristDAO {
    ...
    static {
        try {
            coll = DBManager.getLogDB().getCollection(LogdbKey.LOG_FAVOURIST_COLLECTION);
        } catch (Exception ex) {
            Util.addErrorLog(ex);            
        }
    }
    ...
    public static SizedListData listLog(List<String> listRequestId, List<String> listPartnerId, String fromTime, String toTime,
        ...
        try {
            ...
            if (listRequestId != null) {
                ...
                findObject.append(LogdbKey.LOG_FAVOURIST.REQUEST_ID, inObj);
            }
            if (listPartnerId != null) {
                BasicDBObject inObj = new BasicDBObject("$in", listPartnerId);
                findObject.append(LogdbKey.LOG_FAVOURIST.PARTNER_ID, inObj);
            }
            if (fromTime != null && toTime != null) {
                ...
                ands[0] = new BasicDBObject(LogdbKey.LOG_FAVOURIST.TIME, lte);
                ands[1] = new BasicDBObject(LogdbKey.LOG_FAVOURIST.TIME, gte);
                findObject.append("$and", ands);
            } else {
                if (fromTime != null) {
                    BasicDBObject gte = new BasicDBObject("$gte", fromTime);
                    findObject.append(LogdbKey.LOG_FAVOURIST.TIME, gte);
                }
                if (toTime != null) {
                    BasicDBObject lte = new BasicDBObject("$lte", toTime);
                    findObject.append(LogdbKey.LOG_FAVOURIST.TIME, lte);
                }
            }
            int or = -1;
            ...
            if (sort == 1) {
                sortObj.append(LogdbKey.LOG_FAVOURIST.TIME, or);
            }
            ...
            while (cursor.hasNext()) {
                LogFavourist log = new LogFavourist();
                BasicDBObject dbO = (BasicDBObject) cursor.next();
                String uId = dbO.getString(LogdbKey.LOG_FAVOURIST.REQUEST_ID);
                log.reqId = uId;
                String partnerId = dbO.getString(LogdbKey.LOG_FAVOURIST.PARTNER_ID);
                log.partnerId = partnerId;
                Integer type = dbO.getInt(LogdbKey.LOG_FAVOURIST.TYPE);
                log.type = type;
                String time = dbO.getString(LogdbKey.LOG_FAVOURIST.TIME);
                log.time = time;
