...
import com.vn.ntsc.backend.entity.impl.log.LogCheckOut;
...
public class LogCheckOutDAO {
    ...
    static {
        try {
            coll = DBManager.getLogDB().getCollection(LogdbKey.LOG_CHECK_OUT_COLLECTION);
        } catch (Exception ex) {
            Util.addErrorLog(ex);            
        }
    }
    ...
    public static SizedListData listLog(List<String> listRequestId, List<String> listPartnerId, String fromTime, String toTime,
        ...
        try {
            ...
            if (listRequestId != null) {
                ...
                findObject.append(LogdbKey.LOG_CHECK_OUT.REQUEST_ID, inObj);
            }
            if (listPartnerId != null) {
                BasicDBObject inObj = new BasicDBObject("$in", listPartnerId);
                findObject.append(LogdbKey.LOG_CHECK_OUT.PARTNER_ID, inObj);
            }
            if (fromTime != null && toTime != null) {
                ...
                ands[0] = new BasicDBObject(LogdbKey.LOG_CHECK_OUT.TIME, lte);
                ands[1] = new BasicDBObject(LogdbKey.LOG_CHECK_OUT.TIME, gte);
                findObject.append("$and", ands);
            } else {
                if (fromTime != null) {
                    BasicDBObject gte = new BasicDBObject("$gte", fromTime);
                    findObject.append(LogdbKey.LOG_CHECK_OUT.TIME, gte);
                }
                if (toTime != null) {
                    BasicDBObject lte = new BasicDBObject("$lte", toTime);
                    findObject.append(LogdbKey.LOG_CHECK_OUT.TIME, lte);
                }
            }
            int or = -1;
            ...
            if (sort == 1) {
                sortObj.append(LogdbKey.LOG_CHECK_OUT.TIME, or);
            }
            ...
            while (cursor.hasNext()) {
                LogCheckOut log = new LogCheckOut();
                BasicDBObject dbO = (BasicDBObject) cursor.next();
                String uId = dbO.getString(LogdbKey.LOG_CHECK_OUT.REQUEST_ID);
                log.reqId = uId;
                String partnerId = dbO.getString(LogdbKey.LOG_CHECK_OUT.PARTNER_ID);
                log.partnerId = partnerId;
                String time = dbO.getString(LogdbKey.LOG_CHECK_OUT.TIME);
                log.time = time;
                String ip = dbO.getString(LogdbKey.LOG_CHECK_OUT.IP);
                log.ip = ip;
