 ...
 */
public class AutoQANotifyDAO {
    private static DBCollection coll;

    static {
        try {
            coll = DBManager.getSettingDB().getCollection(SettingdbKey.AUTO_MESSAGE.AUTO_QA_NOTIFY_COLLECTION);
        } catch (Exception ex) {
            Util.addErrorLog(ex);
        }
    }
...
//                Util.addDebugLog("AutoNews check " +flag);
                if (flag == Constant.FLAG.ON) {
                    ...
                    result.put(id, new AutoQANotify(content, url, ip, id, timeStr, query, flag));
//                    Util.addDebugLog("AutoNews check " +result);
    ...
    public static SizedListData getList(Integer flag, Long skip, Long take) throws EazyException {
        ...
        try {
            ...
            while (cursor.hasNext()) {
               ...
               AutoQANotify autoNotify = new AutoQANotify(content, url, id, time, flg, receiverNumber);
    ...
    public static AutoQANotify get(String id) throws EazyException {
        try {
            ...
            AutoQANotify autoNotify = new AutoQANotify(id, content, url, timeStr, query, receiverNumber);
            autoNotify.clickUserNumber = clickedUsersNumber;
            autoNotify.ratioClickedUser = rationClickedUser;
            return autoNotify;
        } catch (Exception ex) {
