 ...
 * @author hoangnh
 */
public class EditEmojiCatDataApi implements IApiAdapter{
    ...
    public Respond execute(Request request, Date time) {
        ...
        Integer flag = (Integer) formData.get(ParamKey.FLAG);
        ...
        if(!inInvalid){
            ...
            JSONObject info = Util.toJSONObject(Helper.editEmojiCategory(token, id, vnName, vnDes, enName, enDes, imageData.fileId, flag));
            Long uploadCode = (Long) info.get(ParamKey.ERROR_CODE);
            if(uploadCode.intValue() == ErrorCode.SUCCESS){
                result = new EntityRespond(ErrorCode.SUCCESS, null);
            }
        }else{
    ...
    public static JSONObject getFormData(Collection<Part> parts){
        ...
        try{
            for(Part part : parts){
                ...
                if(part.getName().equals(ParamKey.FLAG)){
                    String flag = getString(part.getInputStream());
                    data.put(ParamKey.FLAG, Integer.valueOf(flag));
                }
                if(part.getName().equals(ParamKey.FILES)){
                    String mimeType = part.getContentType();
                    Boolean isImage = mimeType.contains("image");
