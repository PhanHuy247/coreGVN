...
import static vn.com.ntsc.staticfileserver.server.respond.impl.AddEmojiApi.getString;
import static vn.com.ntsc.staticfileserver.server.respond.impl.EditEmojiCatDataApi.getFormData;
 ...
 * @author hoangnh
 */
public class EditEmojiApi implements IApiAdapter{
    ...
    public Respond execute(Request request, Date time) {
        ...
        String id = (String) formData.get(ParamKey.ID);
        ...
        if(!inInvalid){
            ...
            if(imageData != null){
                info = Util.toJSONObject(Helper.editEmoji(token, id, catId, code, name, imageData.fileId, imageData.fileType));
            }else{
                info = Util.toJSONObject(Helper.editEmoji(token, id, catId, code, name, null, null));
            }
    ...
    public static JSONObject getFormData(Collection<Part> parts){
        ...
        try{
            for(Part part : parts){
                ...
                if(part.getName().equals(ParamKey.ID)){
                    data.put(ParamKey.ID, getString(part.getInputStream()));
                }
