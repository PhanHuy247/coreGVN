 ...
 * @author RuAc0n
 */
public class LogBeforePurchaseApi implements IApiAdapter {

    @Override
    public Respond execute(Request request) {
        Respond result = new Respond();
        try {
            Util.addInfoLog("Request log before purchase: " + request.toJson());
            ...
            Long deviceType = Util.getLongParam(request.reqObj, ParamKey.DEVICE_TYPE);
...
//                money = PointPacketDAO.getPrice(packetId);
            ...
            TransactionLogDAO.addTransaction(userId, time, point, money, deviceType.intValue(), timePurchase, internalTransactionId, ip, applicationId);
