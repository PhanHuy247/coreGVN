...
//    public Integer indecent;
    ...
    public static UserDB fromDBObject(DBObject dbObj) {
        ...
        String firstPT = (String) dbObj.get(firstPurchaseTimeKey);
        user.firstPurchaseTime = firstPT;
...
//            user.indecent = (Integer) dbObj.get(indecentKey);
        ...
        Integer isPurchaseO = (Integer) dbObj.get(isPurchaseKey);
        Boolean isPurchase = null;
        if (isPurchaseO != null) {
            if (isPurchaseO == 0) {
                isPurchase = false;
            } else if(isPurchaseO == 1) {
                isPurchase = true;
            }
            user.isPurchased = isPurchase;
        }
        
        Boolean havePurchaseO = (Boolean) dbObj.get(havePurchaseKey);
        Boolean havePurchase = null;
        if (havePurchase != null){
            if (havePurchaseO == false) {
                havePurchase = false;
            } else if(havePurchaseO == true) {
                havePurchase = true;
            }
            user.havePurchase = havePurchase;
        }
...
//        }
    ...
    public User getUserInfor() {
        ...
        Boolean isPurchase = this.isPurchased;
        if (isPurchase != null) {
            user.isPurchased = isPurchase;
        }
        ...
        String regDate = this.registerDate;
        if (regDate != null){
            user.registerDate = regDate;
        }
        
        Boolean havePurchase = this.havePurchase;
        if (havePurchase != null) {
            user.havePurchased = havePurchase;
        }
        
        String firstPT = this.firstPurchaseTime;
        if (firstPT != null) {
            user.firstPurchaseTime = firstPT;
        }
        
        String lastPT = this.lastPurchaseTime;
        if (lastPT != null) {
            user.lastPurchaseTime = lastPT;
        }
