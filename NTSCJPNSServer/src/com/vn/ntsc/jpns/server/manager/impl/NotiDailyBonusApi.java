 ...
 */
public class NotiDailyBonusApi implements IApiAdapter{

    @Override
    public void execute(JSONObject obj, Date time) {
        try {
            String api = (String) obj.get(ParamKey.API_NAME);
            String point = obj.get(ParamKey.POINT).toString();
            String toUserid = (String) obj.get(ParamKey.NOTI_USER_ID);
            String ip = (String) obj.get(ParamKey.IP);

            if (point != null && toUserid != null) {
                ...
                if (notification.andgAlert != null && notification.andgAlert == 0) { //OFF push noti
                    msg = MsgUtil.iosPayload_daily_bonus(api, point, null, toUserid, true);
                } else if (notification.andgAlert != null && notification.andgAlert == 1) {//ON push noti
                    msg = MsgUtil.iosPayload_daily_bonus(api, point, null, toUserid, false);
                }
